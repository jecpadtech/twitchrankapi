...
def eggorank():
    ...
    json_data = response.json()
    for x in json_data["players"]:
        if x["gameName"] == "egnarO5":
            return "Eggo is currently ranked #" + str(x["leaderboardRank"])+ " on the leaderboard with " + str(x["numberOfWins"]) + " wins and a ranked rating of " + str(x["rankedRating"])
...
def eggorec():
    ...
    today = datetime.today()
    currentDate = today.strftime("%B %d, %Y")
    response= requests.get("https://api.henrikdev.xyz/valorant/v1/mmr-history/eu/egnarO5/713")
    json_data = response.json()
    for x in json_data["data"]:
        splitString = x["date"].split()
        newDate = "" +splitString[1] + " "+ splitString[2] + " "+ splitString[3]
